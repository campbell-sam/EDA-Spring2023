...
#When you mark each line as 1, 1, 1, RMarkdown will number the list 1. More practice with R code chunk options 1. Gain proficiency with figures, tables (w/`Kable`) table of contents, etc. 1. Debugging knitting issues
...
```{r project.setup, echo=TRUE, error=FALSE, message=FALSE, warning=FALSE, eval=TRUE}
#Import libraries
...
```{r echo=FALSE, fig.align='left', fig.cap="2018 Ozone", fig.height= 2.8, warning=FALSE}

#2018 boxplot for Ozone levels by month
Ozone2018 <- epa_data %>% 
  filter(Year == 2018) %>% 
  drop_na(Ozone) %>% 
  group_by(Month) %>% 
...
```{r echo=FALSE, fig.cap="2019 Ozone", fig.align='left', fig.height= 2.8, warning=FALSE}
#2019 boxplot for Ozone levels by month
Ozone2019 <- epa_data %>% 
  filter(Year == 2019) %>% 
  drop_na(Ozone) %>% 
  group_by(Month) %>% 
...
https://github.com/campbell-sam/EDA-Spring2023 
