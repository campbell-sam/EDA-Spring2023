...
#Use summary function to check for any NAs
...
#Just by looking at the trend line it's hard to see if there is any slope. Appears to be some seasonality in the data, up and down movements equally separated
ggplot(Rainfall_BR_processed, aes(x = Date, y = Rainfall)) +
  geom_line() + 
  geom_smooth(method = "lm")

#Seems to have an upward trend plus seasonality, but trend may not be even over time
...
Rainfall_BR_processed_ts <- 
  ts(Rainfall_BR_processed$Rainfall, 
     start = c(1901,1), frequency = 12)

Temp_BR_processed_ts <- 
  ts(Temp_BR_processed$Temperature_C, 
     start = c(1901,1), frequency = 12)
...
#decompositions
#s.window stands for seasonal window. When creating time series, we already set the window, so stl function will pull the frequency from your time series object (monthly in this case)
Rainfall_BR_processed_decomposed <- 
  stl(Rainfall_BR_processed_ts, s.window = "periodic")

Temp_BR_processed_decomposed <- 
  stl(Temp_BR_processed_ts, s.window = "periodic")

#Visualize decomposed series
plot(Temp_BR_processed_decomposed)

plot(Rainfall_BR_processed_decomposed)
...
Rainfall_components <- as.data.frame(Rainfall_BR_processed_decomposed$time.series[,1:3])

Rainfall_components <- mutate(Rainfall_components,
                              Observed = Rainfall_BR_processed$Rainfall,
                              Date = Rainfall_BR_processed$Date)

Temp_components <- as.data.frame(Temp_BR_processed_decomposed$time.series[,1:3])
