...
```{r}
#lm is linear model. first comes the y (dependent) variable, and then x (independent) variable
irradiance.regression <- lm(PeterPaul.chem.nutrients$irradianceWater ~ PeterPaul.chem.nutrients$depth)

# another, preferred way to format the lm() function
...
#red line should be symmetrical and close to flat, so these plots show an issue in our linear regression
par(mfrow = c(2,2), mar=c(4,4,4,4))
plot(TPregression)
par(mfrow = c(1,1))
