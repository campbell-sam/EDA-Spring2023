...
ggplot() +
  geom_sf(data=huc8_sf, aes(fill=ACRES)) #not finished
...
huc8_sf_challenge <- st_read('./Data/Spatial/NCHUC8.shp') %>% 
  filter(SUBBASIN == "Upper Neuse")

#Create a map
mapview(huc8_sf, col.regions="purple",color = "gray") +
  mapview(huc8_sf_challenge, col.regions = "orange", alpha = 1)
...
## 3.2 Transforming coordinate reference systems of datasets
        #transforming data means going to different projections
