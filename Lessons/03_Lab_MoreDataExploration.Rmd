...
matrix1[4, ] # If you note the row but not the column, you pull up everything in row 4
matrix1[ , 3] # If you note the column but not the row, you pull up everything in column 3
matrix1[c(12, 14)] # 12th and 14th value of matrix 1, if we were pulling it out to 1 dimension
matrix1[c(12:14)] # 12th item through 14th item
matrix1[2:4, 1:3] # slice of rows (2 through 4) and a slice of columns (1 through 3)
...
# Lists ---- 
# Colon in line below allows you to write 2 different r statements on one line
list1 <- list(name = "Maria", mynumbers = vector1, mymatrix = matrix1, age = 5.3); list1
list1[[2]]
# can use either apostrophes or quotation marks, they just need to match opening and closing
list1['mynumbers']
...
```{r DataExercise-1a}
hightemperatures <- c(64,71,72,63,51,52,61,64,67,73) # numeric vector
lowtemperatures <- c(42,49,60,46,32,32,41,47,52,54) # numeric vector
...
```{r DataExercise-1b}
celsiushighs <- (hightemperatures-32)*5/9
celsiuslows <- (lowtemperatures-32)*5/9
```

Combine your four vectors into a data frame and add informative column names.

```{r DataExercise-1c}
temperaturesdataframe <- data.frame(hightemperatures,lowtemperatures,celsiushighs,celsiuslows) #
names(temperaturesdataframe) <- c("High F","Low F","High C","Low C"); View(temperaturesdataframe) #
```
...
```{r DataExercise-1d}
summary(temperaturesdataframe)
sd(temperaturesdataframe)
sd(hightemperatures)
```
