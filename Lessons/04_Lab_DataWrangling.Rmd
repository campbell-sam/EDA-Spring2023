...
#numeric value, so just need the following code with == and raw year value
NTL.phys.data.1999 <- filter(NTL.phys.data, year4 == 1999)

# Exercise 2: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999.
NTL.phys.data.TuesdayLake1990thru1999 <- filter(NTL.phys.data, 
...
NTL.phys.data.Ex3 <- 
  ...
  select(year4, 1990:1998) %>%
  filter(sampledate %m = 07)
#Luana's method
NTL.phys.data.Ex3 <- 
  NTL.phys.data
